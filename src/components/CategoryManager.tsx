...
export default function CategoryManager({ open, onOpenChange }: Props) {
  ...
  const handleAdd = () => {
    ...
    addCategory({ name: newName, icon: newIcon, type: activeTab });
      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }}
            ...
            <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v as CatType); setAdding(false); setEditingId(null); }}>
              ...
              <TabsContent value="expense">{renderCategoryList(filteredCats)}</TabsContent>
              <TabsContent value="income">{renderCategoryList(filteredCats)}</TabsContent>
            </Tabs>
          </motion.div>
        </motion.div>
      )}
