...
export default function CategoryManager({ open, onOpenChange }: Props) {
  const [cats, setCats] = useState(initialCategories.map(c => ({ ...c, active: true })));
  ...
  const startEdit = (cat: typeof cats[0]) => {
    setEditingId(cat.id);
  ...
  const saveEdit = () => {
    setCats(cats.map(c => c.id === editingId ? { ...c, name: editName, icon: editIcon } : c));
    setEditingId(null);
  };

  const addCategory = () => {
    if (!newName.trim()) return;
    setCats([...cats, { id: Date.now().toString(), name: newName, icon: newIcon, active: true }]);
    setNewName('');
    setNewIcon('📦');
    setAdding(false);
  };
  ...
  const toggleActive = (id: string) => {
    setCats(cats.map(c => c.id === id ? { ...c, active: !c.active } : c));
  };
                ...
                <div key={cat.id} className={`flex items-center gap-3 p-3 rounded-lg ${cat.active ? 'bg-secondary' : 'bg-secondary/50 opacity-50'}`}>
                  {editingId === cat.id ? (
                    <div className="flex-1 space-y-2">
                        ...
                        <div className="flex gap-1 flex-wrap">
                          {emojiOptions.slice(0, 9).map(e => (
                            <button
                              ...
                              className={`w-8 h-8 rounded text-sm flex items-center justify-center ${editIcon === e ? 'bg-primary/20 ring-1 ring-primary' : 'hover:bg-accent'}`}
                            >
                              {e}
                            </button>
                          ))}
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 text-sm bg-card rounded-lg px-3 py-1.5 outline-none text-foreground"
                          autoFocus
                        />
                    ...
                    <>
                      ...
                      <button onClick={() => startEdit(cat)} className="p-1 hover:bg-accent rounded transition-colors">
                        <Pencil className="w-3.5 h-3.5 text-muted-foreground" />
                      </button>
                      <button onClick={() => toggleActive(cat.id)} className="p-1 hover:bg-accent rounded transition-colors">
              ...
              <div className="mt-4 p-3 rounded-lg bg-secondary space-y-2">
                <div className="flex gap-1 flex-wrap">
                  {emojiOptions.slice(0, 9).map(e => (
                    <button
                      ...
                      className={`w-8 h-8 rounded text-sm flex items-center justify-center ${newIcon === e ? 'bg-primary/20 ring-1 ring-primary' : 'hover:bg-accent'}`}
                    >
                      {e}
                    </button>
                  ))}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 text-sm bg-card rounded-lg px-3 py-1.5 outline-none text-foreground placeholder:text-muted-foreground/50"
                    autoFocus
                  />
                  <button onClick={addCategory} className="text-xs text-primary font-medium px-2">Agregar</button>
